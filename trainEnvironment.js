    ...
    return new Promise((resolve, reject) => {
        ...
        ffmpegProcess.stdout.on("data", (data) => {
            ...
            if (frameSize == frameMaxSize) {
                resolve(parseRawVideoDataRGB(frameBuffer))
            }
        })
    })
}
...
async function prepareTrainingData(files) {
    ...
    for (let fileName of files) {
        ...
        const preprocessedImage = tf.tensor(image, [environment.Resolution[0], environment.Resolution[1], 3])
            .div(tf.scalar(255))
        //.expandDims();
...
prepareTrainingData(files).then((trainingData) => {
    environment.EnvironmentModel.train(trainingData.xTrain, trainingData.yTrain).then(async () => {
        ...
        /*for (let testIndex = 0; testIndex < files.length; testIndex++) {
            await visualizePredictions(transcodeJsonFile(files[testIndex]), files[testIndex], path.join(__dirname, "tests", `testExpected${testIndex}.png`))
